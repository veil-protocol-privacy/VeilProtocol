...
pub async fn create_ata(payer: &Keypair, client: &RpcClient) {
...
pub fn generate_proof_transfer(
    ...
    let receivers_utxo_pubkey = receivers_master_pubkey
        .iter()
        .map(|receiver_master_pubkey| {
            sha256(vec![
                receiver_master_pubkey.as_slice(),
                random_out.as_slice(),
            ])
        })
        .collect::<Vec<_>>();
    ...
    if sum_in > sum_out {
        output_hashes.push(sha256(vec![
            sender_utxo_pubkey.as_slice(),
            token_id.to_bytes().as_slice(),
            (sum_in - sum_out).to_le_bytes().as_slice(),
        ]));
        new_amounts_out.push(sum_in - sum_out);
        commitment_cipher_texts.push(encrypt(
            sender_master_pubkey,
            utxos_in[0].viewing_public_key(),
            random_out.clone(),
            sum_in - sum_out,
            token_id,
            "sender spare token".to_string(),
            nonce.clone(),
    ...
    amounts_out.iter().enumerate().for_each(|(i, amount_out)| {
        output_hashes.push(sha256(vec![
            receivers_utxo_pubkey[i].as_slice(),
            token_id.to_bytes().as_slice(),
    ...
    receivers_utxo_pubkey
        .iter()
        .for_each(|receiver_utxo_pubkey| {
...
pub fn generate_proof_withdraw(
    ...
    amounts_in: Vec<u64>,
    amount_out: u64,
    sender_spend_key: Vec<u8>,
    sender_view_key: Vec<u8>,
    receiver_spend_key: &Keypair,
    receiver_view_key: &Keypair,
) -> (
    SP1ProofWithPublicValues,
    Vec<Vec<u8>>,
    Option<CommitmentCipherText>,
    Vec<Vec<u8>>,
) {
    let sum_in = amounts_in.iter().sum::<u64>();
    let merkle_proofs = leaves
...
pub fn encrypt(
    ...
    let shared_key = share_key(
        sender_viewing_key.clone(),
        blinded_receiver_pubkey.clone(),
    );
