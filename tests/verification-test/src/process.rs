use crate::util::{
    create_ata, create_deposit_instructions_data_test, generate_proof_transfer,
    generate_proof_withdraw, generate_random_bytes, KeyJson,
};
...
async fn test_process_instruction_transfer() {
    ...
    let receiver_master_pubkey = sha256(vec![
        receiver_spend_key.pubkey().to_bytes().as_ref(),
        receiver_nullying_key.as_slice(),
    ]);

    let transaction_signature = rpc_client
        .request_airdrop(
            &payer_pubkey,
            300 * solana_sdk::native_token::LAMPORTS_PER_SOL,
        )
        .await
        .unwrap();
    loop {
    ...
    // Create account
    create_ata(&depositor_keypair, &rpc_client).await;
    ...
    let (mut deposit_data_2, deposit_utxo_2, deposit_random_2) =
        match create_deposit_instructions_data_test(
            &spl_token::native_mint::ID,
            2 * 10_u64.pow(9),
            spending_key.clone(),
    ...
    let (mut deposit_data_3, deposit_utxo_3, deposit_random_3) =
        match create_deposit_instructions_data_test(
            &spl_token::native_mint::ID,
            2 * 10_u64.pow(9),
            spending_key.clone(),
    ...
    let pre_commitment = PreCommitments::new(
        2 * 10_u64.pow(9),
        spl_token::native_mint::ID.to_bytes().to_vec(),
        deposit_utxo_2.utxo_public_key(),
    );
    ...
    let pre_commitment = PreCommitments::new(
        2 * 10_u64.pow(9),
        spl_token::native_mint::ID.to_bytes().to_vec(),
        deposit_utxo_3.utxo_public_key(),
    );
    let inserted_leaf_3 = hash_precommits(pre_commitment);

    tree.insert(vec![
        inserted_leaf.clone(),
        inserted_leaf_2.clone(),
        inserted_leaf_3.clone(),
    ]);
    ...
    let receiver_master_pubkey_2 = sha256(vec![
        receiver_spend_key_2.pubkey().to_bytes().as_ref(),
        receiver_nullying_key_2.as_slice(),
    ]);
    ...
    let receiver_master_pubkey_3 = sha256(vec![
        receiver_spend_key_3.pubkey().to_bytes().as_ref(),
        receiver_nullying_key_3.as_slice(),
    ]);
    ...
    let receiver_master_pubkey_4 = sha256(vec![
        receiver_spend_key_4.pubkey().to_bytes().as_ref(),
        receiver_nullying_key_4.as_slice(),
    ]);
    ...
    let (proof, nullifiers, ciphertext, utxo_hashes) = generate_proof_transfer(
        ...
        vec![
            deposit_utxo.clone(),
            deposit_utxo_2.clone(),
            deposit_utxo_3.clone(),
        ],
        vec![
            deposit_random.clone(),
            deposit_random_2.clone(),
            deposit_random_3.clone(),
        ],
    ...
    let mut transfer_request =
        TransferRequest::new(proof.bytes().to_vec(), tree.root(), tree_number, ciphertext);
