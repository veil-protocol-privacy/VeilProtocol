...
pub fn process_instruction(
    ...
    match instruction {
        ...
        DarkSolInstruction::HideAssets {commitments: Vec<u8>} => {
            process_hide_asset(program_id, accounts,commitments )?
        }
        DarkSolInstruction::Transfer => process_transfer_asset(program_id, accounts, amount)?,
        DarkSolInstruction::Withdraw => process_withdraw_asset(program_id, accounts, amount)?,
    };
    Ok(())
}